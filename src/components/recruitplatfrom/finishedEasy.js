import { Badge, SwipeAction, List, WhiteSpace, Toast } from 'antd-mobile';
import {Component} from 'react';
import { connect,loading } from 'dva';
require("./waitingEasy.css")

class FinishedEasyC extends Component{
    constructor(props) {
        super(props);  
    }
    ...
    handleRow(rowData,sectionName,EmpId){
        ...
        for(let index=0;index<rowData.length;index++){
            const {status,empName,candidateName,exp,companyName,schoolname,education,specialty,department,position} = rowData[index]
            let rowEle=(
    ...
    render(){
        ...
        const {data,status} = this.props.recruitTasks;
        if((status >= 200 && status < 300)){
            if(loadingAll|loadingList){
                ...
                const {finished,EmpId} = data;
                return(
                    <div  style={{marginTop:"89px",left:"0",zIndex:"0"}}>
                        {this.handleSection(finished,EmpId)}
